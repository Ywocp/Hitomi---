...
const OPTION_ALWAYS_SHOW_SETTINGS_PANE = '始终显示设置窗格'
...
class BrazenBaseSearchEnhancer
{
    ...
    constructor(configuration)
    {
        ...
        this._configurationManager = BrazenConfigurationManager.create(this._uiGen, this._config.tagSelectorGenerator)
            ...
            .addFlagField(OPTION_DISABLE_COMPLIANCE_VALIDATION, '禁用所有搜索过滤器。')
    ...
    _addItemBlacklistFilter(helpText)
    {
        this._configurationManager
            .addFlagField(OPTION_ENABLE_TEXT_BLACKLIST, '应用黑名单规则。')
            .addRulesetField(
    ...
    {
        if (helpText === null) {
            helpText = '指定黑名单标签，每行一个规则。可以使用 "&" 或 "|"。'
        }
    ...
    _createSettingsBackupRestoreFormActions()
    {
        return this._uiGen.createFormActions([
            this._uiGen.createFormButton(
                '备份配置', '备份配置到剪贴板。', () => this._onBackupSettings()),
            this._uiGen.createSeparator(),
            this._uiGen.createFormGroupInput('text')
                .attr('id', 'restore-settings')
                .attr('placeholder', '粘贴设置…'),
            this._uiGen.createFormButton(
                '恢复配置', '从上方的字段恢复配置。', () => this._onRestoreSettings()),
        ], 'bv-flex-column');
    }
    ...
    _createSettingsFormActions()
    {
        return this._uiGen.createFormActions([
            ...
            this._uiGen.createFormButton('保存', '应用并更新已保存的配置。', () => this._onSaveSettings()),
            this._uiGen.createFormButton('重置', '恢复到已保存的配置。', () => this._onResetSettings()),
        ]);
    }
    ...
    _onBackupSettings()
    {
        navigator.clipboard.writeText(this._configurationManager.backup()).then(() => this._uiGen.updateStatus('设置已备份到剪贴板！')).catch(
            () => this._uiGen.updateStatus('设置备份失败！'))
    }
    ...
    _onRestoreSettings()
    {
        ...
        if (!settings) {
            this._uiGen.updateStatus('未提供设置！', true)
            Utilities.sleep(3000).then(() => this._uiGen.resetStatus())
        } else {
            try {
                this._configurationManager.restore(settings)
                this._uiGen.updateStatus('设置已恢复！')
                this._validateCompliance()
            } catch (e) {
                this._uiGen.updateStatus('设置恢复失败！')
            }
        }
    ...
    init()
    {
        if (Utilities.callEventHandler(this._onValidateInit)) {
            ...
            this._uiGen.updateStatus('初次运行已完成。')
